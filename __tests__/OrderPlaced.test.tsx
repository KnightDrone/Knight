  ...
  return jest.fn().mockImplementation(() => {
    return {
      readData: jest.fn().mockImplementation(() => {
        ...
        order.calculateDroneArrivalTime = jest.fn().mockReturnValue(30); // Mocking the arrival time to 30 minutes
        return order;
      }),
      updateData: jest.fn(),
    };
...
describe("OrderPlaced", () => {
  ...
  it("updates completion over time", () => {
    ...
    expect(newParsedWidth).toBeGreaterThanOrEqual(parsedWidth);
    act(() => {
    ...
    expect(newerParsedWidth).toBeGreaterThanOrEqual(newParsedWidth);
  ...
  it("displays the estimated time correctly", async () => {
    const { getByTestId } = render(<OrderPlacedTest />);
    await waitFor(() => {
      const estimatedTime = getByTestId("drone-arrival-time");
      expect(estimatedTime).toBeTruthy();
      expect(estimatedTime.props.children).toBe(30); // Check if the estimated time is displayed correctly
    });
  });
  it("updates the estimated time correctly", async () => {
    const { getByTestId } = render(<OrderPlacedTest />);
    await waitFor(() => {
      const estimatedTime = getByTestId("drone-arrival-time");
      expect(estimatedTime).toBeTruthy();
    ...
    act(() => {
      jest.advanceTimersByTime(10_000); // Advance by 10 seconds
    });
    await waitFor(() => {
      const estimatedTime = getByTestId("drone-arrival-time");
      expect(estimatedTime).toBeTruthy();
      expect(estimatedTime.props.children).toBe(30); // Check if the estimated time is still correct
    });
  });
});
